...
const ChatBot: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-24 right-4 lg:bottom-8 lg:right-8 z-[110]">
      {isOpen ? (
        <div className="bg-white w-[350px] sm:w-[400px] h-[500px] rounded-[2.5rem] shadow-2xl flex flex-col border border-zinc-100 overflow-hidden animate-in fade-in slide-in-from-bottom-10 duration-300">
          ...
          <div className="p-4 bg-white border-t border-zinc-100 flex gap-2">
            <input 
              ...
              className="flex-1 bg-zinc-50 border-none rounded-xl px-4 py-2 text-sm focus:ring-2 focus:ring-violet-500"
            />
            <button 
              onClick={handleSend}
              disabled={!input.trim()}
              className="w-10 h-10 bg-violet-600 text-white rounded-xl flex items-center justify-center hover:bg-violet-700 transition-colors disabled:opacity-50"
            >
        ...
        <button 
          ...
          className="w-16 h-16 bg-violet-600 text-white rounded-full shadow-3xl shadow-violet-600/40 flex items-center justify-center text-2xl hover:scale-110 active:scale-95 transition-all animate-bounce"
        >
